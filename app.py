...
- **Percentage of Transactions:** {len(loss_items)/len(df)*100:.1f}%\n\n"""
    ...
    # --- Distribution ---
        ...
        if target_col and value_col:
            ...
            response += f"\n📌 **Insight:** {dist.loc[dist[value_col].idxmax(), target_col]} dominates with {dist[value_col].max()/total*100:.1f}% share." if total != 0 else "\n📌 **Insight:** No data to compute distribution."
